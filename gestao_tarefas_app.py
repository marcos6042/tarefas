...
paginas = {
    ...
    "Cadastro Subcategoria": ["Categoria (lista de categorias)", "Nome da Subcategoria", "Periodicidade", "subcategorias"],
    "Cadastro Usuário": ["Nome do Usuário", "E-mail", "usuarios"],
    "Nova Tarefa": ["Filial (selecionar razão social e UF)", "Setor", "Subcategoria (selecionar nome da subcategoria)", "Nome da Tarefa", "Competência", "Vencimento", "Usuário", "tarefas"],
    "Baixa de Tarefa": ["Selecionar Tarefa", "Data de Baixa", "Comprovante", "tarefas"]
}

if st.session_state.page in paginas:
    st.container()  # Para abrir corretamente em uma nova página
    ...
    if not df.empty:
        df["Atualizar"] = df.index.map(lambda i: st.button(f'✏️ Atualizar', key=f'edit_{lista}_{i}'))
        df["Excluir"] = df.index.map(lambda i: st.button(f'🗑️ Excluir', key=f'del_{lista}_{i}'))
    if not df.empty:
        st.dataframe(df)
    if st.button("Voltar"):
        st.session_state.page = "menu"
...
if st.session_state.page == "Calendário":
    ...
    df_calendar = pd.DataFrame(st.session_state.tarefas) if st.session_state.tarefas else pd.DataFrame(columns=['Competência', 'Vencimento', 'Nome da Tarefa'])
...
if st.session_state.page == "Lista":
    ...
    df_list = pd.DataFrame(st.session_state.tarefas) if st.session_state.tarefas else pd.DataFrame(columns=['Filial', 'Setor', 'Subcategoria', 'Nome da Tarefa', 'Usuário'])
    st.write(df_list)
    if st.button("Voltar"):
        st.session_state.page = "menu"
        st.rerun()
