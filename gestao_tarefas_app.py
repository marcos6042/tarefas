...
paginas = {
    ...
    "Nova Tarefa": ["Filial (lista de filiais)", "Setor", "Subcategoria (lista de subcategorias)", "Nome da Tarefa", "Competência", "Vencimento", "Usuário", "tarefas"],
...
if st.session_state.page in paginas:
    ...
    st.write(pd.DataFrame(getattr(st.session_state, lista)))
    if st.button("Voltar"):
        st.session_state.page = "menu"
        st.rerun()
