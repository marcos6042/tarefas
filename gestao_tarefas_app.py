...
if st.session_state.page in paginas:
    ...
    if not df.empty:
        df["Atualizar"] = df.index.map(lambda i: st.button("✏️ Atualizar", key=f"edit_{lista}_{i}"))
        df["Excluir"] = df.index.map(lambda i: st.button("🗑️ Excluir", key=f"del_{lista}_{i}"))
    st.write(df)
    if st.button("Voltar"):
        st.session_state.page = "menu"
        st.rerun()
