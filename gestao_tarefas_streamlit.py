...
menu = ["Dashboard", "Empresas", "Usuários", "Tarefas", "Sair"]
choice = st.sidebar.selectbox("Menu", menu)

if choice == "Dashboard":
    st.subheader("📊 Dashboard de Tarefas")
    c.execute("SELECT status, COUNT(*) FROM tarefas GROUP BY status")
    data = c.fetchall()
    if data:
        df = pd.DataFrame(data, columns=["Status", "Quantidade"])
        fig = px.pie(df, names="Status", values="Quantidade", title="Distribuição de Tarefas")
        st.plotly_chart(fig)
    else:
